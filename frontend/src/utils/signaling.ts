...
class HttpSignalingService {
  ...
  async joinRoom(roomId: string, userName: string, role: string) {
    console.log('Joining room via HTTP:', { roomId, userName, role, userId: this.userId });
    
    try {
      const url = `${this.baseUrl}/api/signaling/${roomId}`;
...
export const signalingService = new HttpSignalingService();
